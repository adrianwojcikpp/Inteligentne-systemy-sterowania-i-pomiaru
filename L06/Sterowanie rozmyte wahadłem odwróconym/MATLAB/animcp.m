function [sys, x] = animcp(t, x, u, flag, action)
...
%   $Revision: 1.15 $  $Date: 2002/04/02 21:25:36 $
...
if ~isempty(flag) && flag == 2
    ...
    new_pole = pole*exp(-1i*theta) + pos;
...
%------>tmp = [startH stopH pauseH contH stepH timeH -1 -1 -1 -1];
        tmp = [startH stopH pauseH contH stepH timeH nanH nanH nanH nanH];
...
%------>tmp = [signalH dispmodeH -1 -1 -1 -1 -1 -1 -1 -1];
        tmp = [signalH dispmodeH nanH nanH nanH nanH nanH nanH nanH nanH];
...
%------>tmp = [cartH poleH forceH refH -1 -1 -1 -1 -1 -1];
        tmp = [cartH poleH forceH refH nanH nanH nanH nanH nanH nanH];
        set(AnimCpFigH, 'userdata', [get(AnimCpFigH, 'userdata'); tmp]);
